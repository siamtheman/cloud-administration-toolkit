...
resource "azurerm_resource_group" "rg" {
  name     = "rg-cloud-engineering-lab"
  location = "West US"
}

# Create the Virtual Network
resource "azurerm_virtual_network" "vnet" {
  name                = "totallyurbanfashion"
...
resource "azurerm_subnet" "subnet" {
  name                 = "totallyurbanfashion-basement"
  resource_group_name  = azurerm_resource_group.rg.name
...
resource "azurerm_log_analytics_workspace" "security_law" {
  name                = "law-cyber-incident-pipeline"
...
# Adds Microsoft Sentinel onboarding state.
resource "azurerm_sentinel_log_analytics_workspace_onboarding" "sentinel" {
  workspace_id = azurerm_log_analytics_workspace.security_law.id
}

# Append this to connect Entra ID Sign-In Logs directly to your Sentinel Workspace
